...
export default function newCubicles() {
  ...
  useEffect(() => {
    fetch("/products.json")
      .then((res) => res.json())
      .then((data) => setCubicles(data["New Cubicles"] || []));
  }, []);
  ...
  if (filter === "price") {
    ...
    filteredCubicles = [...filteredCubicles].sort(
  ...
  const paginatedCubicles = filteredCubicles.slice(
    (page - 1) * perPage,
    page * perPage
  );
    ...
    <div className="bg-white px-4">
      <div className="container mx-auto py-10 sm:py-16 md:py-20 xl:py-[120px] flex flex-col gap-12">
        <h2 className="text-center text-[#1E1D1D]">
          Explore Our New Cubicle Options
        </h2>
        <div className="flex flex-col gap-6">
              ...
              <Select value={filter} onValueChange={setFilter}>
                <SelectTrigger className="w-full px-4 py-2 border border-[#D2D2D5] bg-transparent focus:ring-0 focus:outline-none">
                  <SelectValue
                    ...
                    className="text-[#1D1F2C] text-[15px] font-medium"
                  />
                  <span className="ml-auto flex items-center">
                    ...
                    >
                      <g clipPath="url(#clip0_5268_2361)">
                        <path
                          d="M0 5.5C0 5.23478 0.105357 4.98043 0.292893 4.79289C0.48043 4.60536 0.734784 4.5 1 4.5H19C19.2652 4.5 19.5196 4.60536 19.7071 4.79289C19.8946 4.98043 20 5.23478 20 5.5C20 5.76522 19.8946 6.01957 19.7071 6.20711C19.5196 6.39464 19.2652 6.5 19 6.5H1C0.734784 6.5 0.48043 6.39464 0.292893 6.20711C0.105357 6.01957 0 5.76522 0 5.5ZM3 10.5C3 10.2348 3.10536 9.98043 3.29289 9.79289C3.48043 9.60536 3.73478 9.5 4 9.5H16C16.2652 9.5 16.5196 9.60536 16.7071 9.79289C16.8946 9.98043 17 10.2348 17 10.5C17 10.7652 16.8946 11.0196 16.7071 11.2071C16.5196 11.3946 16.2652 11.5 16 11.5H4C3.73478 11.5 3.48043 11.3946 3.29289 11.2071C3.10536 11.0196 3 10.7652 3 10.5ZM8 14.5C7.73478 14.5 7.48043 14.6054 7.29289 14.7929C7.10536 14.9804 7 15.2348 7 15.5C7 15.7652 7.10536 16.0196 7.29289 16.2071C7.48043 16.3946 7.73478 16.5 8 16.5H12C12.2652 16.5 12.5196 16.3946 12.7071 16.2071C12.8946 16.0196 13 15.7652 13 15.5C13 15.2348 12.8946 14.9804 12.7071 14.7929C12.5196 14.6054 12.2652 14.5 12 14.5H8Z"
                          fill="#1D1F2C"
                        />
                      </g>
                      <defs>
          ...
          <div className="flex items-center justify-center gap-2 mt-8">
            <button
              ...
              disabled={page === 1}
              className={`px-4 sm:px-6 py-2 sm:py-3 border border-[#1A1AFF] rounded transition-all text-[#1A1AFF] bg-white text-base font-normal flex items-center gap-2 ${
                page === 1
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <button
                key={i}
                onClick={() => setPage(i + 1)}
                className={`w-14 h-12 border border-[#1A1AFF] rounded transition-all text-base font-normal flex items-center justify-center ${
                  page === i + 1
                    ? "bg-[#1A1AFF] text-white"
                    : "bg-white text-[#1A1AFF] hover:bg-[#f0f3ff]"
                }`}
              >
                {i + 1}
              </button>
            ...
            <button
              ...
              className={`px-4 sm:px-6 py-2 sm:py-3 rounded transition-all text-white bg-[#2B3DFF] text-base font-normal flex items-center gap-2 ${
                page === totalPages
                  ? "opacity-50 cursor-not-allowed"
                  : "hover:bg-[#f0f3ff]"
              }`}
